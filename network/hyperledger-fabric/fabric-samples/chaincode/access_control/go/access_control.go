...
func (t *SimpleChaincode) issueAdvertisement (stub shim.ChaincodeStubInterface, args []string) pb.Response{
    ...
    contractJSONasString := `{"TxID": "` + string(txID) + `","txType": "` + txType + `","title": "` + title + `","description": "` + description + `","price": "` + price +`","dataType": "` + dataType + `","ipAddress": "` + ipAddress + `","orgID": "` + orgID + `"}`
	contractJSONasBytes:= []byte(contractJSONasString)
...
func (t *SimpleChaincode) issueBuy (stub shim.ChaincodeStubInterface, args []string) pb.Response{
    ...
    contractJSONasString := `{"TxID": "` + string(txID) + `", "advertisementTxId": "` + advertisementTxId + `","txType": "` + txType + `","ipAddress": "` + ipAddress + `","orgID": "` + srcOrgID + `"}`
	contractJSONasBytes:= []byte(contractJSONasString)
